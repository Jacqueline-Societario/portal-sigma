...
- 28/07/2026 | 89c8dc0 | feat: adiciona Declaracao de Inexistencia de Habite-se ao modulo Declaracoes (painel dedicado, Goiania/Outros municipio, data automatica) | Claude/Jacqueline (implantado via tar+scp, md5 2/2 OK, backup pre-deploy em backups/deploy/, restart ok, servico estavel 46s+, HTTP 302 em /declaracoes/ e 200 em /login apos restart)
- 28/07/2026 | 708ad6d | fix: bloco de assinatura da Declaracao Habite-se — legenda "Assinatura" (itálico/cinza) + 6 linhas em branco antes do nome da empresa, conforme comparação com modelo de referência da Jacqueline | Claude/Jacqueline (implantado via tar+scp, md5 OK, backup pre-fix em backups/deploy/, restart ok, servico estavel, HTTP 302/200 apos restart)
